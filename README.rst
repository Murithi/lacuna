...
Lacuna is a project to help close the gap between communities that are on the move, and those still trying to figure out how to get started.
...
lacuna relies extensively on environment settings which **will not work with Apache/mod_wsgi setups**. It has been deployed successfully with both Gunicorn/Nginx and even uWSGI/Nginx.

For configuration purposes, the following table maps the 'lacuna' environment variables to their Django setting:
...
DJANGO_DEFAULT_FROM_EMAIL               DEFAULT_FROM_EMAIL          n/a                                            "lacuna <noreply@lacuna.com>"
DJANGO_SERVER_EMAIL                     SERVER_EMAIL                n/a                                            "lacuna <noreply@lacuna.com>"
DJANGO_EMAIL_SUBJECT_PREFIX             EMAIL_SUBJECT_PREFIX        n/a                                            "[lacuna] "
DJANGO_ALLOWED_HOSTS                    ALLOWED_HOSTS               ['*']                                          ['lacuna.com']
======================================= =========================== ============================================== ======================================================================
...
.. code-block:: bash
    ...
    cd lacuna
    celery -A lacuna.taskapp worker -l info
...
To run the tests, enter the lacuna/tests directory and run the following commands::
...
application stacks on one host, remove the port setting entirely and add `VIRTUAL_HOST=lacuna.com` to your env file.
This pass all incoming requests on `nginx-proxy` to the nginx service your application is using.

.. _nginx-proxy: https://github.com/jwilder/nginx-proxy

Postgres is saving its database files to `/data/lacuna/postgres` by default. Change that if you wan't
...
Place it in `/etc/supervisor/conf.d/lacuna.conf` and run::

    supervisorctl reread
    supervisorctl start lacuna
